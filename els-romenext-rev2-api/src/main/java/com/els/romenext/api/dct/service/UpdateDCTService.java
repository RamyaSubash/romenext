...
public class UpdateDCTService {
	
	private static Logger log = Logger.getLogger(UpdateDCTService.class);
	
	public Response runService( UpdateDCTRequest req, Long metaid ) {
		...
		if( g == null ) {
			log.error("The GROUP to Updated Not Exist: " + req.getUpdateDct().getId() );
...
////		System.out.println( "romeType : " + romeType );
		...
		if(!coreServices.typeNodeExistsById(RomeNodeClassEnum.TYPE, req.getDctId(),  metaId.getId() )) {
			log.error("The Type to Updated Not Exist: " + req.getDctId() );
		...
		RomeType dctOld = typeDao.get( req.getDctId() );
		...
		// ensure this is a DCT
		if( dctOld.getClassificationEnum() != RomeTypeClassificationEnum.DCT ) {
			log.error("The Type to Updated is not DCT");
			responseBuilder = ErrorResponse.build(ApiResponseState.FAILURE, RomeNextResponseCodeEnum.DCT_UNKNOWN, null).getResponseBuilder();
			return responseBuilder.build();
		}
		...
		RomeGroupType groupType = groupTypeDao.findByGroupAndType( g,  dctOld );
		
		if( groupType == null ) {
			// this user does not have access to this type!!
			log.error("The user does not have access to this type!: " + dctOld );
		...
		if ( req.getUpdateDct().getName() != null && !dctOld.getName().equalsIgnoreCase( req.getUpdateDct().getName() )) {
			Boolean dupCheck = coreServices.checkIfNameAlreadyExistsForType( dctOld, req.getUpdateDct().getName(), metaId );
			if( dupCheck == null || dupCheck ) {
				log.error("The New Name Has Been Occupied: " + req.getUpdateDct().getName() );
				responseBuilder = ErrorResponse.build(ApiResponseState.FAILURE, RomeNextResponseCodeEnum.DCT_DUPLICATE, null).getResponseBuilder();
				return responseBuilder.build();
			} else {
				// update the name
				
				dctOld.setName( req.getUpdateDct().getName() );
		...
		if( req.getUpdateDct().getProperties() != null ) {
			
			for( Property p : req.getUpdateDct().getProperties() ) {
		 ...
		 * 
		 */
		if( req.getUpdateDct().getDecoProperties() != null ) {
			...
			List<Property> decoProperties = req.getUpdateDct().getDecoProperties();
		...
		Node node = builder.build( dctOld ); 
