package com.els.romenext.api.node.service.preference;

import javax.ws.rs.core.Response;

import org.apache.log4j.Logger;

import com.els.romenext.api.node.req.preference.AddPreferenceToNodeRequest;
...
public class AddPreferenceToNodeService {
	
	private static Logger log = Logger.getLogger(AddPreferenceToNodeService.class);
	
	private NodeCoreServices nodeCoreServices = null;
	private RelationshipCoreServices relationshipCoreServices = null;
	
	public AddPreferenceToNodeService () {

	}
	
	public AddPreferenceToNodeService (NodeCoreServices nodeCoreServices, RelationshipCoreServices relationshipCoreServices) {
		this.nodeCoreServices = nodeCoreServices;
		this.relationshipCoreServices = relationshipCoreServices;
	}
	
	public Response runService(AddPreferenceToNodeRequest req, Long repoid ) {
...
////		if (CollectionUtils.isEmpty( req.getDecorators() )  ) {
...
////			return responseBuilder.build();
////		}
//		if( CollectionUtils.isEmpty( req.getPreferences() ) ) {
//			log.error("Failed to update node because no update preferences found");
//			responseBuilder = ErrorResponse.build(ApiResponseState.FAILURE, RomeNextResponseCodeEnum.PREFERENCE_UPDATE_FAILED_NOTHINGTOUPDATE, null).getResponseBuilder();
//			return responseBuilder.build();
//		}
//		
//		RomeTypeDao typeDao = new RomeTypeDao( req.namespace );
//		RomeType type = typeDao.findById( req.getTypeId(), metadataRepo.getMetadata() );
//		
...
//		RomeGroupType groupType = rgtDao.findByGroupAndType( g,  type );
//		
//		if( groupType == null ) {
...
//		
//		
//		Node inputNode = NodeBuilder.build_SystemAndType( req.namespace, type, req.getProperties(), req.getSysProperties() );
//		
//		
////		Node inputNode = NodeBuilder.build(null, req.getTypeId(), req.getProperties(), null, null);
...
//		if (CollectionUtils.isNotEmpty(req.getPreferences() )) {
//			tmpUpdatedNodes = nodeCoreServices.updateNodeWithTypeProperties(inputNode, req.getPreferences(), metadataRepo);
//		}
//		
