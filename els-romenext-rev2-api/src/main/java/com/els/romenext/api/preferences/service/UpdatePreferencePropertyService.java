...
public class UpdatePreferencePropertyService {
	
	private static Logger log = Logger.getLogger(UpdatePreferencePropertyService.class);

	public Response runService( UpdatePreferencePropertyRequest req, Long metadataId ) {
		...
		for( Property p : req.getProperties() ) {
			...
			if( prefValMap.containsKey( propId ) ) {
				// update
				RomePreferenceGroupTypePropertyValue oldVal = prefValMap.get( propId );
				
				oldVal.setValue( p.getValue().toString() );
...
//				return responseBuilder.build();
		...
		try {
			...
			for (RomePreferenceGroupTypePropertyValue p : updates) {
				prefPropDao.save( p );
			}
