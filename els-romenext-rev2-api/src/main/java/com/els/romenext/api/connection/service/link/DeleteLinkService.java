...
public class DeleteLinkService {
	...
	private static Logger log = Logger.getLogger(DeleteLinkService.class);
...
//		this.relServices = new Neo4jCoreServiceFactory().getRelServices( metadataRepo.getIp(), metadataRepo.getUsername(), metadataRepo.getPassword(), req.namespace );
	...
	public DeleteLinkService ( Neo4jRelationshipServices relServices  ) {
...
//		this.neo4jNodeServices = neo4jNodeServices;
	...
	public Response runService(DeleteLinkRequest req, Long metadataId) {

		ResponseBuilder responseBuilder;
		
		ApiLoginUtils loginCheck = new ApiLoginUtils();
		boolean authorized = loginCheck.isAuthorized( req.namespace );
		
		if( !authorized ) {
			// user is probably not logged in
			log.error("Call came into a not authorized session" + req.namespace );
		 ...
		 */
		if( rc.getClassification() != RomeRuleClassificationEnum.LINK.getInternalId() ) {
			log.error("Attempting to delete a NON-LINK connection");
			responseBuilder = ErrorResponse.build(ApiResponseState.FAILURE, RomeNextResponseCodeEnum.LINK_DELETE_ATTEMPTED_TO_DELETE_CONN, null).getResponseBuilder();
			return responseBuilder.build();
		}
		...
		/**
		 * FOR LINKS: We never delete the rule!
		 */
