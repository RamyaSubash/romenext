...
public class AddPreferenceRequest extends GroupRequest {
	
	private static Logger log = Logger.getLogger(AddPreferenceRequest.class);
	...
	public String validateRequest(JSONObject json) {
		...
		for (int i = 0; i < jsonArray.length(); i++) {
			...
			empty = RomeJSONUtils.findEmptyJson(prop, "name", "propertyType");
			if (empty != null) {
				return empty;
			}
		}
