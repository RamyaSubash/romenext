...
import com.els.romenext.api.dct.resp.GetAllDCTAndConnectionsByGroupResponse;
 ...
 *
 */
public class GetAllDCTAndConnectionsByGroupService {
	
	private static Logger log = Logger.getLogger(GetAllDCTAndConnectionsByGroupService.class);
	
	public Response runService( GetAllDCTAndConnectionsByGroupRequest req, Long metaId ) {
		ResponseBuilder responseBuilder;
		
		MetadataContainerDao mcDao = new MetadataContainerDao( req.namespace );
		MetadataContainer mc = mcDao.get(metaId);	
		if (mc == null) {
...
//		NodeUtils nodeUtils = new NodeUtils(username);
		...
		Map<Long, Relationship> connectionMap = new HashMap<>();
...
//		List<RomeType> rtList = new ArrayList<RomeType>();
		for (RomeGroupType rgt : rgtList) {
			...
			if( rgt.getRomeType().getClassificationEnum() == RomeTypeClassificationEnum.DCT  ) {
				...
				List<RomeConnection> connTmp = rcDao.findByTypeClassificationMetadata( RomeRuleClassificationEnum.PARENTCHILD, rgt.getRomeType(), mc );
				
				if( connTmp != null ) {
					for( RomeConnection c : connTmp ) {
						if( !connectionMap.containsKey( c.getId() ) ) {
							connectionMap.put( c.getId().longValue(), RelationshipBuilder.build( c ) );
						}
					}
				}
...
//		}
		...
		GetAllDCTAndConnectionsByGroupResponse response = new GetAllDCTAndConnectionsByGroupResponse();
		response.build( typeFullList.values(), connectionMap.values() );
