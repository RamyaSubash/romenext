function DesignPhysicalRenderer() {

	this.divholderId;
	
	DesignPhysicalRenderer.gridx = 1;
	DesignPhysicalRenderer.gridy = 1;
	DesignPhysicalRenderer.gridz = 1;
	
	/**
	 * Redefine the origin point
	 */
	DesignPhysicalRenderer.offsetX = 0;
	DesignPhysicalRenderer.offsetY = 0;
	DesignPhysicalRenderer.offsetZ = 0;
...
//		ModelUtils.global_model_fns( "create", "physical", DesignPhysicalRendererCrud.modelCreate );
//		ShapeUtils.global_shape_fns( "create", "physical", DesignPhysicalRendererCrud.shapeCreate );
//		ModelPropertyUtils.global_modelproperty_fns( "create", "physical", DesignPhysicalRendererCrud.modelPropertyCreate );
	...
	this.initView = function() {
		...
		// disable other views? 		
		// var logicalDecoBody = document.getElementById("logical_design_view");
		// logicalDecoBody.style.display = "block";
		
		// enable the view?
		var physview = document.getElementById( "physical_design_view_id" );
//		document.getElementById( "physical_design_view_id" ).style.display = "block";
		
		if (document.getElementById( "physical_design_view_id" ).style.display != 'block') {
			
			// load all needed data
			// initialize the 3d view
			// generate the view

			DesignPhysicalRenderer.defaultCamera = 800;
			DesignPhysicalRenderer.requestedGrid = "OFF";
			DesignPhysicalRenderer.activeGrid = "ON";
			...
			if (this.checkInitialValues() == true) {
				...
				var inputs = '<p style="color:red"><b>No Type Selected</b></p>';
...
//				document.getElementById('toolbar_romenext').innerHTML = inputs; 
				...
				var inputs = '<p style="color:red"><b>Multiple Types Selected</b></p>';
				console.log("multiple types selected for the physical design");
				$('#console-log').append("<p style='color:red'>Missing initial variables (metadata/current type)</p>");
				$('#toolbar_romenext').empty();
//				document.getElementById('toolbar_romenext').innerHTML = inputs; 
			}
...
//    	document.getElementById("pv3d").style.display = "block"; 	
		var physicalDecoBody = document.getElementById( "physical_design_view_id" );
		physicalDecoBody.style.display = "block";
		
		if (document.getElementById('grid-models') == undefined || document.getElementById('grid-models') == null) {
			
			var gridModels = document.createElement('div');
			gridModels.id = 'grid-models';
			gridModels.style.visibility = 'hidden';
			gridModels.innerHTML += '<div class="box  box-solid box-info "  style="min-height: 100px;overflow: auto;">'
								+ '<div class="box-header with-border">'
								+ '<h2 class="box-title" id="Infotitle">Model Details</h2>'
								+ '<div class="pull-right box-tools">'
								+ '<button class="btn btn-box-tool" data-widget="collapse" title="Collapse" ><i class="fa fa-minus"></i></button>'
								+ '</div></div><div class="box-body" id="model_info" style="display:none;"></div></div>';
			physicalDecoBody.appendChild(gridModels);
		
		}
		$("#grid-models").draggable().resizable();
		
		if (document.getElementById('pv3d') == undefined || document.getElementById('pv3d') == null) {
			var cy = document.createElement('div');
			cy.id = 'pv3d';
			cy.style.display = 'block';
			physicalDecoBody.appendChild(cy);
		}
...
////								+ '<button class="btn btn-box-tool" data-widget="collapse" title="Collapse" ><i class="fa fa-minus"></i></button>'
////								+ '</div></div><div class="box-body" id="model_info" style="display:none;"></div></div>';
////			physicalDecoLn.appendChild(gridModels);
////		
////		}
//		
...
//  verifying that a type is selected and a metadata 
    this.checkInitialValues = function(){     	
    	if(selectedMetaData == null || listTypeIds.length != 1  || typeMapViaId.length == 0){
    		console.log(" no type selected for the physical design");
    		$('#console-log').append("<p style='color:red'> Missing initial variables( metadata/current Type)</p>");	
    		return false;
    	} else {
	...
	this.activeToolbar = function() {
		...
		inputs += '<td id="design_3d_three_selected_type_td" style="display:block"><b>' + typeMapViaId[listTypeIds[0]].name + '(' + listTypeIds[0] + ')' + '</b></td>';
		inputs += '<td width="50px" onclick="( new DesignPhysicalRenderer() ).internal_showAddModel3DDialog()"><img title="Add Model" src="/webgui/assets/img/icons/add.png"></td>';

		if (curModels.length > 0) {

			inputs += '<td width="50px">';

			inputs += '<select id="select_model" onchange="( new DesignPhysicalRenderer() ).internal_loadModelShapes()"><option value="select model">select model Redux...</option>';
			...
			inputs += '<td id="design_3d_three_front_td"  width="25px" style="display:none"><input id="design_3d_three_front" type="button" style="border:1;visibility:visible;color:green" onclick="( new DesignPhysicalRenderer() ).internal_front();" value="Front"></td>';
			inputs += '<td id="design_3d_three_back_td"   width="25px" style="display:none"><input id="design_3d_three_back" type="button" style="border:1;visibility:visible;color:" onclick="( new DesignPhysicalRenderer() ).internal_back();" value="Back"></td>';
			inputs += '<td id="design_3d_three_left_td"   width="25px" style="display:none"><input id="design_3d_three_left" type="button" style="border:1;visibility:visible;color:" onclick="( new DesignPhysicalRenderer() ).internal_left();" value="Left"></td>';
			inputs += '<td id="design_3d_three_right_td"  width="25px" style="display:none"><input id="design_3d_three_right" type="button" style="border:1;visibility:visible;color:" onclick="( new DesignPhysicalRenderer() ).internal_right();" value="Right"></td>';
			inputs += '<td id="design_3d_three_top_td"    width="25px" style="display:none"><input id="design_3d_three_top" type="button" style="border:1;visibility:visible;color:" onclick="( new DesignPhysicalRenderer() ).internal_top();" value="Top"></td>';
			inputs += '<td id="design_3d_three_bottom_td" width="25px" style="display:none"><input id="design_3d_three_bottom" type="button" style="border:1;visibility:visible;color:" onclick="( new DesignPhysicalRenderer() ).internal_bottom();" value="Bottom"></td>';
			inputs += '<td id="design_3d_three_grid_td"   width="25px" style="display:none"><input id="design_3d_three_grid" type="button" style="border:1;visibility:visible;color:" onclick="( new DesignPhysicalRenderer() ).internal_grid_toggle();" value="Toggle Grid"></td>';

			inputs += '<td id="draw_line_td"    width="220px" style="display:none"><img id="img_grid" title="Construction Line" src="/webgui/assets/img/icons/grid.png" onclick="toDrawGrid3D()"><span id="xyz_limitations" style="display:none">x(red)<input type="checkbox" id="x_limitation" value="x">y(green)<input type="checkbox" id="y_limitation" value="y">z(blue)<input type="checkbox" id="z_limitation" value="z"></span></td>';
...
//			inputs += '<td width="50px" onchange="( new DesignPhysicalRenderer() ).updateGrid();">X:<input type="text" id="gridx"/> </td>';
//			inputs += '<td width="50px" onchange="( new DesignPhysicalRenderer() ).updateGrid();">Y:<input type="text" id="gridy"/> </td>';
//			inputs += '<td width="50px" onchange="( new DesignPhysicalRenderer() ).updateGrid();">Z:<input type="text" id="gridz"/> </td>';
//							
//			inputs += '<td width="25px" id="offsetx_td" style="display:none" onchange="( new DesignPhysicalRenderer() ).updateOffset();">X:<input type="text" size="10" id="offsetx"/> </td>';
//			inputs += '<td width="25px" id="offsety_td" style="display:none" onchange="( new DesignPhysicalRenderer() ).updateOffset();">Y:<input type="text" size="10" id="offsety"/> </td>';
//			inputs += '<td width="25px" id="offsetz_td" style="display:none" onchange="( new DesignPhysicalRenderer() ).updateOffset();">Z:<input type="text" size="10" id="offsetz"/> </td>';
...
//		document.getElementById('toolbar_romenext').innerHTML = inputs; 
	...
	this.generatePath = function() {
		...
		list += "<li><a href='#'>Physical Design</a></li>";
...
//		DesignPhysicalRenderer.gridx = $('#gridx').val();
//		DesignPhysicalRenderer.gridy = $('#gridy').val();
//		DesignPhysicalRenderer.gridz = $('#gridz').val();
//	}
//
//	this.updateOffset = function() {
//		DesignPhysicalRenderer.offsetX = Number( $('#offsetx').val() );
//		DesignPhysicalRenderer.offsetY = Number( $('#offsety').val() );
//		DesignPhysicalRenderer.offsetZ = Number( $('#offsetz').val() );
...
//				of : "#main-content"
			...
			buttons : {
				"Add Model 3D" : function() {
					(new DesignPhysicalRenderer()).internal_createNewModel3D(dialog.find("form"));
				},
				Cancel : function() {
					dialog.dialog("close");
					grayOut(false);
		...
		dialog.find("form").on("submit", function(event) {
			...
			(new DesignPhysicalRenderer()).internal_createNewModel3D(this);
		});
	...
	this.internal_createNewModel3D = function(form) {
		...
		var successFunction = function( data ) {
			...
			(new DesignPhysicalRenderer()).activeToolbar();
			
			PhysicalInterfaceUtils.recreateModelDropdownMenu3D();
			
			(new DesignPhysicalRenderer()).internal_generateAxisConstructionLines();
	...
	this.internal_generateAxisConstructionLines = function() {
		...
		var jsonDataY = '{"model":' + curModel + ', "shape": "LINE", "isConstruction":true, ' + 
	    				...
	    				'"height":0, "depth":0, "width":0, "angle":0, "parent":-2, "shapeType":3}';
	...
	this.internal_loadModelShapes = function() {
		...
		this.internal_resetAllDrawingTools();
...
//		curModel= this.getModelIdByName(document.getElementById("select_model").value);
		...
		ModelShapeUtils.displayShapes3D(curModelShapes);   
...
//		document.getElementById("design_3d_three_bottom").style.color = "";
	...
	this.internal_resetAllDrawingTools = function() {

		// disable these for now
		
//		document.getElementById("img_grid").src = "/webgui/assets/img/icons/grid.png";
...
//		
//		document.getElementById("img_cntr").src = "/webgui/assets/img/icons/cntr.png";
//		document.getElementById("img_eye").src = "/webgui/assets/img/icons/hide.png";
//		document.getElementById("img_ajst").src = "/webgui/assets/img/icons/ajst.png";
//		document.getElementById("img_move").src = "/webgui/assets/img/icons/hand.png";
	...
	this.internal_init = function() {
		...
		DesignPhysicalRenderer.activeCamera = "FRONT";
		DesignPhysicalRenderer.requestedCamera = "FRONT";
		...
		camera.position.set( 0, - DesignPhysicalRenderer.defaultCamera, 0 );
...
//		// camera.toFrontView();
		...
		if( DesignPhysicalRenderer.activeGrid === "ON" ) {
...
//			renderer.render(scene, camera);
			...
			DesignPhysicalRenderer.internal_render();
		};
		...
		window.addEventListener('resize', (new DesignPhysicalRenderer()).onWindowResize, false);
	}
	 ...
	 */
	this.cx = function( x ) {
		return x + Number( DesignPhysicalRenderer.offsetX );
	};
	
	this.cy = function( y ) {
		return y + Number( DesignPhysicalRenderer.offsetY );
	};
	
	this.cz = function( z ) {
		return z + Number( DesignPhysicalRenderer.offsetZ );
	};
...
//	}
	...
	this.internal_grid_toggle = function() {
		if( DesignPhysicalRenderer.requestedGrid === "ON" ) {
			DesignPhysicalRenderer.requestedGrid = "OFF";
		} else {
			DesignPhysicalRenderer.requestedGrid = "ON";
		}
	};
	...
	this.internal_front = function() {
		...
		( new DesignPhysicalRenderer() ).internal_resetAllPlaneButtonColors();
		if (camera != null) {
			camera.position.set( 0, - DesignPhysicalRenderer.defaultCamera, 0 );
		}
		DesignPhysicalRenderer.activeCamera = "FRONT";
//		document.getElementById('design_3d_three_front').style.color = 'green';
	...
	this.internal_back = function() {
		DesignPhysicalRenderer.requestedCamera = "BACK";
		plane3DName = "xz";
		...
		DesignPhysicalRenderer.activeCamera = "BACK";
//		document.getElementById('design_3d_three_back').style.color = 'green';
	}

	this.internal_left = function() {
		DesignPhysicalRenderer.requestedCamera = "LEFT";
		plane3DName = "yz";
		
		( new DesignPhysicalRenderer() ).internal_resetAllPlaneButtonColors();
		if (camera != null) {
			camera.position.set( - DesignPhysicalRenderer.defaultCamera, 0, 0 );
		}
		DesignPhysicalRenderer.activeCamera = "LEFT";
//		document.getElementById('design_3d_three_left').style.color = 'green';
	}

	this.internal_right = function() {
		DesignPhysicalRenderer.requestedCamera = "RIGHT";
		plane3DName = "yz";
		...
		DesignPhysicalRenderer.activeCamera = "RIGHT";
//		document.getElementById('design_3d_three_right').style.color = 'green';
	}

	this.internal_top = function() {
		DesignPhysicalRenderer.requestedCamera = "TOP";
		plane3DName = "xy";
		
		( new DesignPhysicalRenderer() ).internal_resetAllPlaneButtonColors();
		if (camera != null) {
			camera.position.set( 0, 0, DesignPhysicalRenderer.defaultCamera );
		}
		DesignPhysicalRenderer.activeCamera = "TOP";
//		document.getElementById('design_3d_three_top').style.color = 'green';
	}

	this.internal_bottom = function() {
		DesignPhysicalRenderer.requestedCamera = "BOTTOM";
		plane3DName = "xy";
		...
		DesignPhysicalRenderer.activeCamera = "BOTTOM";
	...
	this.onViewMouseMove = function(event) {
		...
		if (action3D == "line") {
			if (mouseStatus == 1 && new3dElement != null) {
				...
				var oldX = new3dElement.position.x;
				var oldY = new3dElement.position.y;
				var oldZ = new3dElement.position.z;
				...
				if (drawingLineType == 1) {  // parallel to x
					new3dElement.position.x = xMovingDiff;
				} else if (drawingLineType == 3) {    // parallel to y
					new3dElement.position.y = yMovingDiff;
				} else if (drawingLineType == 5) {    // parallel to y
					new3dElement.position.z = zMovingDiff;
				} else {
					console.log("Wrong Line Type");
				}
							
				if (xLimited) {
					new3dElement.position.x = oldX;
				}
				if (yLimited) {
					new3dElement.position.y = oldY;
				}
				if (zLimited) {
					new3dElement.position.z = oldZ;
				}
			...
			showSelectionIndicator(intersects);	
				...
				if(mouseStatus == 1 && selected3DLine != null){
					...
					if (selected3DLine.shapeType == 1) { // parallel to x
						...
						selected3DLine.value = selected3DLine.position.y;
						selected3DLine.tochange = 'y';
					} else if (selected3DLine.shapeType == 5) { // parallel to z
						selected3DLine.position.x = 0;
						selected3DLine.value = selected3DLine.position.y;
						selected3DLine.tochange = 'y';
					} else {
...
//              Release  Variables        
			...
			xMovingDiff = lines3d.position.x;
			yMovingDiff = lines3d.position.y;
			zMovingDiff = lines3d.position.z;
				...
				if (intersects.length > 0) {
					...
					for (var i = 0; i < intersects.length; i++) {
						...
						if (new3dElement == null) {
							...
							new3dElement = ModelShapeUtils.drawLine3D(line3dColor, [intersects[i].object.geometry.vertices[0].x, intersects[i].object.geometry.vertices[0].y, intersects[i].object.geometry.vertices[0].z], [intersects[i].object.geometry.vertices[1].x, intersects[i].object.geometry.vertices[1].y, intersects[i].object.geometry.vertices[1].z]);
							
//							if (intersects[i].object.shapeType == 1) {
//								new3dElement = ModelShapeUtils.drawLine3D(line3dColor, [0, intersects[i].point.y, intersects[i].point.z], [gridSize, intersects[i].point.y, intersects[i].point.z]);
//							} else if (intersects[i].object.shapeType == 3) {
//								new3dElement = ModelShapeUtils.drawLine3D(line3dColor, [0, 0, 0], [intersects[i].point.x, gridSize, intersects[i].point.z]);
//							} else if (intersects[i].object.shapeType == 5) {
//								new3dElement = ModelShapeUtils.drawLine3D(line3dColor, [intersects[i].point.x, intersects[i].point.y, 0], [intersects[i].point.x, intersects[i].point.y, gridSize]);
//							} else {
//								console.log("Wrong Data");
//							}
							drawingLineType = intersects[i].object.shapeType;
							new3dElement.shapeType = intersects[i].object.shapeType;
							
							if (new3dElement != null) {
								new3dElement.dbParent = intersects[i].object.dbId;
								new3dElement.position.copy(new THREE.Vector3(new3dElement.position.x + xMovingDiff, new3dElement.position.y + yMovingDiff, new3dElement.position.z + zMovingDiff));
...
//										new3dElement = ModelShapeUtils.drawLine3D(line3dColor, [0, intersects[i].point.y, 0], [gridSize, intersects[i].point.y, 0]);
//										drawingLineType = 1;
...
//										new3dElement = ModelShapeUtils.drawLine3D(line3dColor, [intersects[i].point.x, 0, 0], [intersects[i].point.x, gridSize, 0]);
//										drawingLineType = 3;
...
//											new3dElement = ModelShapeUtils.drawLine3D(line3dColor, [0, 0, 0], [gridSize, 0, 0]);
//											drawingLineType = 1;
//											new3dElement.shapeType = 1;
//										} else if (intersects[i].point.y != 0) {
//											new3dElement = ModelShapeUtils.drawLine3D(line3dColor, [0, 0, 0], [0, gridSize, 0]);
//											drawingLineType = 3;
...
//										new3dElement = ModelShapeUtils.drawLine3D(line3dColor, [0, 0, intersects[i].point.z], [0, gridSize, intersects[i].point.z]);
//										drawingLineType = 3;
//										new3dElement.shapeType = 3;  
//									} else if (intersects[i].object.geometry.vertices[1].z == gridSize && intersects[i].object.geometry.vertices[0].y != 0) {
//										new3dElement = ModelShapeUtils.drawLine3D(line3dColor, [0, intersects[i].point.y, 0], [0, intersects[i].point.y, gridSize]);
//										drawingLineType = 5;
...
//											new3dElement = ModelShapeUtils.drawLine3D(line3dColor, [0, 0, 0], [0, 0, gridSize]);
//											drawingLineType = 5;
//											new3dElement.shapeType = 5;   
//										} else {
//											console.log("Wrong Parent Line Selected");
...
//										new3dElement = ModelShapeUtils.drawLine3D(line3dColor, [0, 0, intersects[i].point.z], [gridSize, 0, intersects[i].point.z]);
//										drawingLineType = 1;
//										new3dElement.shapeType = 1;
//									} else if (intersects[i].object.geometry.vertices[1].z == gridSize && intersects[i].object.geometry.vertices[0].x != 0) {
//										new3dElement = ModelShapeUtils.drawLine3D(line3dColor, [intersects[i].point.x, 0, 0], [intersects[i].point.x, 0, gridSize]);
//										drawingLineType = 5;
//										new3dElement.shapeType = 5;  
//									} else {
//										console.log("Wrong Data");
				...
				if (intersects.length > 0) {
					for (var i = 0; i < intersects.length; i++) {	
						if (foundIntersect == false) {
							...
							// PhysicalInterfaceUtils.verifiedLinePlane(intersects[i].object) && intersects[i].object.isConstruction
							if (intersects[i].object.isConstruction) {
//							    intersects[0].object.material.color.setHex(Math.random() * 0xffffff);
												...
												if (selectedElementInters < 1) {                              // no previous intersection saved
													...
													if(verifyNewIntersection3D(selectedElementInters[selectedElementInters.length-1]  , selectedElementInter) ){
														...
														if (selectedLine.length == 3 && selectedElementInters.length == 2) {
															...
															contourNew.position.copy(new THREE.Vector3(contourNew.position.x + xMovingDiff, contourNew.position.y + yMovingDiff, contourNew.position.z + zMovingDiff));
					...
					if (intersects.length >0) {	
						for (var i = 0; i < intersects.length; i++) {	
							if (foundIntersect == false) {
								// !verifyLineIsAxis(intersects[0].object) &&  PhysicalInterfaceUtils.verifiedLinePlane(intersects[0].object) && intersects[0].object.isConstruction
								if(!verifyLineIsAxis(intersects[0].object) && intersects[0].object.isConstruction){ 
										mouseStatus = 1;
									    intersects[0].object.material.color.setHex(Math.random() * 0xffffff);
...
//									}
					...
					if (mouseStatus == 0) {
							...
							if (intersects.length >0) {						
								for (var i = 0; i < intersects.length; i++) {	
										if (foundIntersect == false) {
											// PhysicalInterfaceUtils.verifiedLinePlane(intersects[0].object) && intersects[0].object.isConstruction
											if(intersects[0].object.isConstruction){ 
...
//	    camera.updateProjectionMatrix();
	...
	this.getModelIdByName = function(name) {                // Retrieve the ID for a model by its name
	...
	this.getShapeById = function(shapeId) {
		for (var key in lines3d.children) {
			if (lines3d.children[key].dbId == shapeId) {
				return lines3d.children[key];
			}
		}
		return null;
	};
...
DesignPhysicalRenderer.internal_render = function() {
	
	
	var switchedCamera = false;

	if( DesignPhysicalRenderer.activeCamera === DesignPhysicalRenderer.requestedCamera ) {
		
	} else {
		switchedCamera = true;
		var current = DesignPhysicalRenderer.defaultCamera;
		( new DesignPhysicalRenderer() ).internal_resetAllPlaneButtonColors();
		if( DesignPhysicalRenderer.requestedCamera === "FRONT" ) {
			if (camera != null) {
				camera.position.set( 0, - DesignPhysicalRenderer.defaultCamera, 0 );
			}
//			camera.position.set( 0, - DesignPhysicalRenderer.defaultCamera, 0 );
			DesignPhysicalRenderer.activeCamera = "FRONT";
			document.getElementById('design_3d_three_front').style.color = 'green';
		} else if( DesignPhysicalRenderer.requestedCamera === "BACK" ) {
			if (camera != null) {
				camera.position.set( 0, DesignPhysicalRenderer.defaultCamera, 0 );
			}
//			camera.position.set( 0, DesignPhysicalRenderer.defaultCamera, 0 );
			DesignPhysicalRenderer.activeCamera = "BACK";
			document.getElementById('design_3d_three_back').style.color = 'green';
		} else if( DesignPhysicalRenderer.requestedCamera === "LEFT" ) {
...
//			camera.position.set( - DesignPhysicalRenderer.defaultCamera, 0, 0 );
			DesignPhysicalRenderer.activeCamera = "LEFT";
			document.getElementById('design_3d_three_left').style.color = 'green';
		} else if( DesignPhysicalRenderer.requestedCamera === "RIGHT" ) {
			if (camera != null) {
				camera.position.set( DesignPhysicalRenderer.defaultCamera, 0, 0 );
			}
//			camera.position.set( DesignPhysicalRenderer.defaultCamera, 0, 0 );
		...
		} else if( DesignPhysicalRenderer.requestedCamera === "TOP" ) {
			if (camera != null) {
				camera.position.set( 0, 0, DesignPhysicalRenderer.defaultCamera );
			}
//			camera.position.set( 0, 0, DesignPhysicalRenderer.defaultCamera );
		...
		} else if( DesignPhysicalRenderer.requestedCamera === "BOTTOM" ) {
			if (camera != null) {
				camera.position.set( 0, 0, - DesignPhysicalRenderer.defaultCamera );
			}
//			camera.position.set( 0, 0, - DesignPhysicalRenderer.defaultCamera );
		...
		} else if( DesignPhysicalRenderer.requestedCamera === "PERS" ) {
	...
	if( DesignPhysicalRenderer.activeGrid !== DesignPhysicalRenderer.requestedGrid || switchedCamera ) {
		// we have to make sure the grid is visible
		if( DesignPhysicalRenderer.requestedGrid === "ON" || switchedCamera ) {
...
//			scene.remove( scenegrid );
			...
			if( DesignPhysicalRenderer.activeCamera === "FRONT" || DesignPhysicalRenderer.activeCamera === "BACK" ) {
				scenegrid.rotation.x = Math.PI ;
				
			} else if( DesignPhysicalRenderer.activeCamera === "TOP" || DesignPhysicalRenderer.activeCamera === "BOTTOM" ) {
		...
		} else if( DesignPhysicalRenderer.requestedGrid === "OFF" ) {
			if (scene != null) {
				scene.remove( scenegrid );
			}
		...
		DesignPhysicalRenderer.activeGrid = DesignPhysicalRenderer.requestedGrid;
