...
GlobalHTMLUtils.msgDeleteConn = function() {}

GlobalHTMLUtils.msgDeleteType = function(edges, typeId) {
	var textEdge = "";
	if (edges.length > 0) {
		textEdge = " <br/>Note: the  " + edges.length + "  connections (In or Out) to this Type will be deleted also."
	}
	var text = "<p style='color:red'>This process is irreversible and will delete <span class='badge' style='color:black; background:"
		+ typeMapViaId[typeId].color + ";'> " + typeMapViaId[typeId].name + "</span> from all users.</p>";
	text = text + textEdge;
	text += "Please confirm to delete ?";
}

GlobalHTMLUtils.msgDeleteTypeWithInst = function(nbNodes, typeId) {
	var text = "<span class='badge' style='color:black; background:" + typeMapViaId[typeId].color + ";'> " + typeMapViaId[typeId].name
		+ "</span> currently has " + nbNodes + " instances derived from it. Only type that have no instances can be deleted. Please delete the instances first."
