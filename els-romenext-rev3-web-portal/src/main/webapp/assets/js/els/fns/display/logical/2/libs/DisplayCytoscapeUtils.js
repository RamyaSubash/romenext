...
//	var element = DisplayCytoscapeUtils.formatNode(jsonData);
	var element =  CytoNodeUtils.formatNode(jsonData);
...
//		element = DisplayCytoscapeUtils.formatNode(nodeMap[key]);
		element =  CytoNodeUtils.formatNode(nodeMap[key]);
...
//		element = DisplayCytoscapeUtils.formatNode(nodes[key]);
		element = CytoNodeUtils.formatNode(nodes[key]);	
		elements.push(element);
	}
...
//			element = DisplayCytoscapeUtils.formatNode(value2);
			...
			element = CytoNodeUtils.formatNode(value2);
...
//				irvCy.pan({x:230, y:100});
		...
		if (element.group == 'nodes') {
			...
			cy.$('#' + element.data.id + '').data("label", element.data.label);
...
DisplayCytoscapeUtils.clickANode = function (uuid) {
	...
	var node = irvCy.filter(function(element, i){
		if (element.isNode()) {
...
DisplayCytoscapeUtils.clickAEdge = function (uuid) {
	...
	var edge = irvCy.filter(function(element, i){
		if (element.isEdge()) {
			if (element.data().sysProperties.uuid.value == uuid) {
				return element;
			}
...
DisplayCytoscapeUtils.buildQtip = function (event, elementType) {
