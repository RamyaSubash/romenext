...
var defaults = {
  ...
  handleNodes: 'node[classification!="link"]', // selector/filter function for whether edges can be made from a given node
  ...
  nodeParams: function nodeParams(sourceNode, targetNode) {
    ...
    // return element object to be passed to cy.add() for intermediary node
	  ...
	  console.log("target is : "+targetNode);
    return {};
  },
  ...
  complete: function complete(sourceNode, targetNode, addedEles) {
    // fired when edgehandles is done and elements are added
	  console.log("Call save edges API");
...
function makeEdges() {
  ...
  var classes = preview ? 'eh-preview' : 'edge[classification = "parentchild"]';
...
//    }, options.edgeParams(source, target, 0)), classes));
    ...
    originType = Number(source.id()); // retrieve  Id
	destType   = Number(target.id());
