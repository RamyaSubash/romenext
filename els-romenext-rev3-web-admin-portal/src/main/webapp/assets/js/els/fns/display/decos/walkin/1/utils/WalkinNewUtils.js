 ...
 * 
 */
function WalkinNewUtils() {
	
};

	WalkinNewUtils.createNode = function( typeId ) {
		...
		WalkinNewUtils.findAllTypesRelated( typeId );  
		  	
		WalkinNewUtils.filterRelatedNodes(typeId  );

		console.log(" elementFound contains "+elementFound );
			
		// build the page with all nodes to create 
	
		WalkinNewUtils.addFormCreateNode (typeId, workDiv, listAllNodes, historyNode, elementFound ); 
		...
		var footer  = '<tr><td colspan="2"></td></tr><tr><td colspan="2"><font color="red">Red Color Property</font> are Mandatory </td></tr>';
		 footer += 	'<tr align="center"><td colspan="2"><input type="button"  name="submit" class="btn btn-primary btn-xs"   id="addNodeInst_submit_' + typeId+ '" value="Add  '+name+'" onclick="WalkinNewUtils.saveNodeInfo('+typeId+');"/>';		
	...
	WalkinNewUtils.findAllTypesRelated = function ( typeId ){
			if(!typeId  ){
			...
			elementFound  = [];		 
		    elementFound  = [typeId];
		    elementFound  = elementFound.concat( WalkinNewUtils.findAllRelatedNodes(elementFound));  // a bug one entry with undefined
    ...
    WalkinNewUtils.findAllRelatedNodes = function (  parentId   ){
...
//                   	       if( $.inArray(value.target.toString(), childrenIds)  ==  -1 ) {
    	...
    	if (childrenIds.length == 0) {
    		...
    		if( curThreshold <=  thresholdNodeCreation ) {
        		...
        		return childrenIds.concat( WalkinNewUtils.findAllRelatedNodes(childrenIds));
    		}  		
    	}  	
    	
    }
	
    WalkinNewUtils.getNodesLinkedToWalkin = function ( nodes ,instHolderDiv){
		 ...
		 for ( var i = 0; i < nodes.length; i++){
			 ...
			 if( nodes[i].startNodeUuid != null){
				 if( nodes[i].endNodeUuid == null  && nodes[i].status  == 'initial'  ){
					 ...
					 jsonData = WalkinNewUtils.buildJsonGetNodesFromEntryNodes ( nodes[i].startNodeUuid, nodes[i].endType  );
					 // call API					 
					 WalkinNewUtils.getNodesFronEntry ( jsonData, i, nodes  );
					 ...
					 // complete the nodes info
						 if( !$.isEmptyObject(historyNode)){
							 ...
							 if (historyNode.length >= 1  ){ 
								 ...
								 WalkinUtils.addDivsForAWalkinDetails (instHolderDiv, historyNode[0] );
						
								 var typeInfo2 = typeMapViaId[typeId2];
								 ( new WalkinManagerRender() ).displayViewEditProperties( typeInfo2, historyNode[0],  nodeMap[historyNode[0]]  );
								 ...
								 for ( var k = 1; k< historyNode.length; k++){
									 var addNode = {};
									   ...
									   WalkinUtils.addDivsForAWalkinDetails (instHolderDiv, historyNode[k] );
									   
									   var typeInfo2 = typeMapViaId[typeId2];
									   ( new WalkinManagerRender() ).displayViewEditProperties( typeInfo2, historyNode[k],  nodeMap[historyNode[k]]  );
        ...
        if (childrenNodes.length == 0) {
    		return childrenNodes;
        		...
        		return childrenNodes.concat( WalkinNewUtils.getNodesLinkedToWalkin(childrenNodes,instHolderDiv ));  	 		
    	}  		
	...
	WalkinNewUtils.filterRelatedNodes = function ( typeId  ){
		var nodes = [];
		// remove  WALKIN
		...
		for ( var i=0; i <elementFound.length ; i++){
			for ( j= i+1 ; j< elementFound.length ; j++){
				...
				if(foundCon.length == 1) {
					 ...
					 if ( foundCon[0].source.toString()== elementFound[i]){
						 nodePos = 'asParent';
					 }else if (foundCon[0].target.toString()== elementFound[i] ){
						 nodePos = 'asChild';
					 }
				  	 listAllNodes.push({ currNodeType : elementFound[i], currNodePos : nodePos , relNodeType : elementFound[j], typeNode : mustNode , connection : foundCon[0] });
//				  	 listRelatedNodes.push({ startType: list[i] , startNodeUuid: null, endType: list[j]  , endNodeUuid: null,  startTypePos :nodePos  , status: 'initial' });
						...
						// TO DO!  in case of multiple connections
...
//################################## Other version of filter used for selectNode ##################################################
    WalkinNewUtils.filterRelatedNodes2 = function ( typeId, list  ){
		var nodes = [];
	...
	WalkinNewUtils.addFormCreateNode = function (typeId, workDiv, relatedNodes, listHistory, list ){     // list [ , , , ] contains all types Related; relatedNodes -- how nodes are related        
			...
			inputContentList +=  '<fieldset" ><legend>'+typeMapViaId[typeId].name +' Details: </legend> ';
				...
				if ( !$.isEmptyObject(list) )  {  				
					for (var node = 0; node < list.length ; node++ ) {				
						inputContentList += WalkinNewUtils.loopThroughRelatedNodesToDisplay( list[node]  , relatedNodes,listHistory  );						
					}
				}      	    
			 ...
			 WalkinNewUtils.addNodeDetails( $("#addNodeInst_"+typeId), typeId);
			 WalkinNewUtils.setDates();
			 
		}
	
	WalkinNewUtils.loopThroughRelatedNodesToDisplay = function ( typeId, relatedNodes , listHistory ){
	    		...
	    		if( relatedNodes[node].currNodeType == typeId  ){
			        ...
			        if ( endNode == relatedNodes[node].connection.source  ){			    
					    ...
					    if( ableToCreate == true )	{    		
					   		line += '<input type="radio" class="createOptions" name="mustConn_'+endNode+'"   value="create" onclick="WalkinNewUtils.showCreateDiv('+endNode+', '+typeId+' )"/>Create New ';		    					    	
					   	}	    		    				    	
				    	line += '<input id="connect_parents_in_creation" type="radio" name="mustConn_'+endNode+'"    value="use" onclick="WalkinNewUtils.showCreateDiv('+endNode+', '+typeId+' )" />Search ';			    			    			    	
	...
	WalkinNewUtils.addNodeDetails = function( appendToDiv, typeId ){
	...
	WalkinNewUtils.showCreateDiv = function (parent, typeId ){
	    	if(!typeId || !parent ){
	    	...
	    	if(valueSelected == 'create' ){
	    		...
	    		WalkinNewUtils.addNodeDetails( $("#nodeCreateDiv_"+parent) , parent  );
	    		...
	    		WalkinNewUtils.generateNodesSearchForm($("#nodeDisplayDiv_"+parent), parent);
	...
	WalkinNewUtils.cleanPreviousResult = function ( typeId ){
		...
		if (prevSearchResult != undefined || prevSearchResult != null) {	
			...
			var typeIdsRel  = WalkinNewUtils.getTypeFromList (typeId);             
	...
	WalkinNewUtils.generateNodesSearchForm = function( workspace, typeId ) {
	    	if(!typeId ){
	    	...
	    	WalkinNewUtils.cleanPreviousResult ( typeId );
	        ...
	        inputContent += '<tr><td colspan="2"><input type="button" name="submit" class="btn btn-primary btn-xs" id="search_node_submit_' + typeId + '" value="Search" onclick="WalkinNewUtils.searchNodes(\'' + workspace.attr('id') + '\', \'tableNodeSearch_' + typeId + '\', '  + typeId + ');"/></td></tr></table>';    
	                                                                                                   
	    	workspace.append(inputContent);

	    };
	    
	WalkinNewUtils.searchNodes = function ( outterWorkspaceId, innerWorkspaceId, typeId){
	    	NodeUtils.searchNodesByProperties(outterWorkspaceId, innerWorkspaceId, typeId );
	  
	    	$('#node_list_'+typeId).attr('onclick', 'WalkinNewUtils.viewNode('+typeId +')');
	    	$('#returnToSearch_'+typeId).attr('onclick', 'WalkinNewUtils.generateNodesSearchForm($("#nodeDisplayDiv_'+typeId+'"),'+ typeId+')');
	}
	    
	WalkinNewUtils.viewNode = function ( typeId){
		...
		WalkinNewUtils.displayInfoNodeSelected(typeId, nodeUuid, value, div );
		...
		if( typeIdsRel ){
			...
			WalkinNewUtils.getSubTree (nodeUuid, typeId, typeIdsRel  );
			...
			if(listInstUuids[0]){
				...
				WalkinNewUtils.displayInfoNodeSelected(typeN, listInstUuids[0], value, div2 );
	...
	WalkinNewUtils.displayInfoNodeSelected = function (typeId,  key , value, where ){
	    	if(!key  ){
	...
	WalkinNewUtils.listAll = function ( propToSearch   ){
		...
		$.each( entryNodes, function(key, value) {
			...
			if(!uuid ){
				console.log(" no Walkin uuid  given "+ uuid );
			...
			if(!rowColor){
			 ...
			 for( var i=0; i <tableRows.length; i++){	 
				 var propToShow = WalkinNewUtils.findValueProp (value, tableRows[i].id.toString() );
				 inputs += "<td>  "+ propToShow+"  </td>";
			    }
			inputs += "<td><button type='button'  id='"+uuid+"'  onclick=\"( new WalkinManagerRender() ).selectedNode('" + uuid + "')\"  >View details</td> </tr>";		
	     });
...
//=======================================================================================================================    
	WalkinNewUtils.saveNodeInfo = function( typeId  ) {
		...
		foundNodes.forEach(function( value ){
			...
			if( value.action == 'create'){
				...
				WalkinNewUtils.SaveNodeCreated (value.type, properties ); 
		...
		if (document.getElementById('reservation_detail_'+resUuid) == undefined || document.getElementById('reservation_detail_'+resUuid) == null) {		
			WalkinUtils.createAppendHTMLEntity('div', 'reservation_detail_'+resUuid , 'reservation_detail', 'visible', 'inline-block', '', bodyDiv);
		}
					
		( new WalkinManagerRender() ).selectedNode( resUuid );	
	...
	WalkinNewUtils.SaveNodeCreated = function ( nodeTypeId, properties ){
	...
	WalkinNewUtils.getTypeFromList = function(typeId  ){
		var typeIdsRel = [];
		for ( var i=0; i<elementFound.length; i++){
	...
	WalkinNewUtils.getSubTree = function (nodeUuid, typeId ,typeIdsRel ){
	...
	WalkinNewUtils.buildJsonGetNodesFromEntryNodes = function ( nodeUuidStart, typeIdFinish   ){
		var jsonData    = {}; 
		var properties  = []; 
		var typeIds     = [];
		var entryNode   = {};
	...
	WalkinNewUtils.getNodesFronEntry = function ( jsonData, ind , nodes   ){
		historyNode = [];
		var successFunction = function( data ) {
			if($.isEmptyObject(data)||    $.isEmptyObject(data.nodes)){      // at least the node itself is returned if it has no Parent/Children/Sibling
...
//	
//	WalkinNewUtils.findPropertyIdType = function ( typeId , propId){
...
//	WalkinNewUtils.findFirstProperty = function ( typeId ){
//		 var namePropertyId = null;
//		 if(typeId){
//			 var props = typeMapViaId[typeId].typeProperties;
//			 if( props.length >= 1 )  {
...
//	 }
	...
	WalkinNewUtils.findValueProp = function ( data , propId ){
	...
	WalkinNewUtils.simulateClick  = function ( text ) {
...
//		  var a = document.getElementsByClassName( text ); 
	...
	WalkinNewUtils.setDates = function(){
...
//	    to = $( "#Departure" )
	...
	WalkinNewUtils.updateWalkin_cancel = function ( key ){	
...
//	
//	WalkinNewUtils.buildNodeDetails = function ( nodeUuid,  data){
//		 if(!nodeUuid ){
