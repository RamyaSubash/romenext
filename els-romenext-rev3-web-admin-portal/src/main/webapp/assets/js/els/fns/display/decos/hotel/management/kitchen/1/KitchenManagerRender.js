/**
 * For kitchen reservations, use case follows:
 * 
 * 1. Enter Entry date
 ...
 * @returns
 */
function KitchenManagerRender() {
	
	this.divholderId;
	this.DEFAULT_TYPE_NAME = "KITCHEN";
	this.DEFAULT_TYPE_KITCHEN = null;
	...
	// reset the Interface  -- Ensuring that only proper division will be displayed
		...
		// print out the kitchens	
		if( this.DEFAULT_TYPE_KITCHEN != null ) {
	...
	this.enableFormView = function() {
		...
		var kitchenManagerDecoView = document.getElementById(activeDecos_BODY[ this.divHolderId ]);
        
		kitchenManagerDecoView.style.display = "block";	
	...
	this.structureDiv = function() {
		...
		var kitchenManagerDecoView = document.getElementById(activeDecos_BODY[ this.divHolderId ]);
		kitchenManagerDecoView.style.display = "block";
		kitchenManagerDecoView.innerHTML = '';
		
		// generate the Kitchen bar
		if (document.getElementById('kitchen_mgr_kitchen_list_id') == undefined || document.getElementById('kitchen_mgr_kitchen_list_id') == null) {
			//                          elementType, elementId,                 className,     visibility, display, innerHtml
			KitchenUtils.createAppendHTMLEntity('header', 'kitchen_mgr_kitchen_list_id' , 'panel-heading', 'visible', 'block', '', kitchenManagerDecoView);
		}
		
		if (document.getElementById('kitchen_mgr_kitchen_inst_id') == undefined || document.getElementById('kitchen_mgr_kitchen_inst_id') == null) {
			//                          elementType, elementId,                 className,     visibility, display, innerHtml
			KitchenUtils.createAppendHTMLEntity('header', 'kitchen_mgr_kitchen_inst_id' , 'panel-heading', 'visible', 'block', '', kitchenManagerDecoView);
			$('#kitchen_mgr_kitchen_inst_id').style = "vertical-align:top;";
		}
		
		
		// generate body Division 
		var bodyDiv ;
		if (document.getElementById('kitchen_body') == undefined || document.getElementById('kitchen_body') == null) {			
			KitchenUtils.createAppendHTMLEntity('div', 'kitchen_body' , 'cy', 'visible', 'flex', '', kitchenManagerDecoView);
			bodyDiv = $( "#kitchen_body");
		}
		...
		if(document.getElementById('work_addInstance') == undefined || document.getElementById('work_addInstance') == null) {		
			KitchenUtils.createAppendHTMLEntity('div', 'work_addInstance'  , 'work_addInstance', 'hidden', 'none', '', bodyDiv);			
		}	
    ...
    this.initializeTypeBar = function() {
		...
		var displayTypeBar_v2 = document.getElementById('kitchen_mgr_kitchen_list_id');
		...
		inputs += "<tr><td >Current Kitchens</td>";
	    inputs += "<td class='create_icon' onclick=\"( new KitchenManagerRender() ).createKitchen();\"><img  id='img_create' title='Create Kitchen'   src='"+img_path+ "design_icons/create.png'></td></tr>";	    
	...
	this.initData = function() {
		...
		if( this.DEFAULT_TYPE_KITCHEN == null ) {
			...
			var tmpKitchen = this.DEFAULT_TYPE_NAME;
			var foundKitchen = null;
			...
			this.DEFAULT_TYPE_KITCHEN = foundKitchen;
		}
		
		if( this.DEFAULT_TYPE_KITCHEN == null ) {					
			document.getElementById('path').innerHTML = "<p style='color:red'>Could not found the Info for this DECO  !!</p>"+tmpKitchen;
			$('#console-log').append("Could not found the Info for this DECO  !!"+tmpKitchen);
			console.log(" Could not found the Info for this DECO  !!"+ tmpKitchen);
			return false;
			
		}else {	
				// we set the default to the kitchen type
				GlobalUtils.setGlobalTypeSelected( this.DEFAULT_TYPE_KITCHEN.id );					
				// we should have all the kitchen nodes
				// note: we treat the kitchen's as the ENTRY NODE for this deco
				var tmpKitchens = GlobalTypeInstanceUtils.getAllInstances( this.DEFAULT_TYPE_KITCHEN.id );
				
				this.ENTRY_NODES = tmpKitchens;
				return true;					
		}
	};
		
	this.printKitchens = function() {	
											
//		build List of Type Kitchen Instances available if ANY	
		var kitchenInfoDiv = document.getElementById('kitchen_mgr_kitchen_inst_id');
			
//		output the entry kitchens
		var inputs = "<table id='kitchenlist'><tr>";
		...
		$.each( entryNodes, function(key, value) {
			...
			if(!uuid ){
				console.log(" no Kitchen uuid  given "+ uuid );
...
//			create the subdivision for that node 		
			bodyDiv = $( "#kitchen_body");		
			if (document.getElementById('kitchen_detail_'+uuid) == undefined || document.getElementById('kitchen_detail_'+uuid) == null) {		
				KitchenUtils.createAppendHTMLEntity('div', 'kitchen_detail_'+uuid , 'kitchen_detail', 'visible', 'inline-block', '', bodyDiv);
			}
//			find the Name of Kitchen 
			var tmpName = NodeUtils.findNameInst(value);
			console.log("This should be the name : " + tmpName );
																	
			inputs += "<td ><button type='button' style='color:black; background:"+value.color+"' id='"+uuid+"'  onclick=\"( new KitchenManagerRender() ).selectedKitchen('" + uuid + "')\"  >"+ tmpName;
			inputs += "</button></td>";
	     });
			inputs +="</tr></table>";
			kitchenInfoDiv.innerHTML = inputs;			
	}	
	
	this.addKitchenInst = function(uuid ){
		if(!uuid ){
...
//		create the subdivision for that node 		
		...
		var inputs = '';															
		inputs += "<td ><button type='button' style='color:black; background:"+nodeMap[uuid].color+"' id='"+uuid+"'  onclick=\"( new KitchenManagerRender() ).selectedKitchen('" + uuid + "')\"  >"+ tmpName;
		inputs += "</button></td>";
	
		$('#kitchenlist').find('td:last').after( inputs);					
	}

	this.setDefaultsForDivs = function( key ){
		if(!key){
			console.log(" no Kitchen key  given "+ key);
			console.log(" Can not set the default divisions for the node ");
			return;
		}
		KitchenUtils.setDefault_ChildrenDiv(key);			
		KitchenUtils.setDefault_ParentDiv(key);	
		KitchenUtils.setDefault_SiblingDiv(key);
//		KitchenUtils.setDefault_LinktoDiv(key);
//		KitchenUtils.setDefault_ConnectToDiv(key);
	}
...
//	BUILT ADD CHILD NODE FORM  Based on SELECTED CHILD TYPE
	this.addChildNodeForm = function( nodeUuid ) {
		...
		if($( ".addChildNodeRow" ).length == 0) {
			...
			inputChild   += '<td>Select Type: <select id="select_child_type_'+nodeUuid+'"  onchange="KitchenUtils.buildTypeProperty( \'child\',  this.value,\''+ nodeUuid +'\');" ></select> </td>';
	
			inputChild   += '<td id="td_' + nodeUuid + '"></td>';
			inputChild   += '<td><input type="button" name="add" id="addChil_'+nodeUuid+'"   class="btn btn-primary btn-xs" value="Add" onclick="( new KitchenManagerRender() ).addChildNode(\''+ nodeUuid + '\');">';		
			inputChild   += '<input type="button" name="cancel" value="cancel"  onclick="KitchenUtils.cancelAddChild()"></td></tr>';
				
			tableNodeChildDiv.append(inputChild);		
	
			KitchenUtils.buildTypeList(nodeUuid, 'Children');
		} else {
...
//	BUILT ADD PARENT NODE FORM  Based on SELECTED CHILD TYPE
	this.addParNodeForm = function( nodeUuid ) {
		...
		if($(".addParentRow").length == 0) {
			...
			inputPar   += '<td>Select Type: <select id="select_parent_type_'+nodeUuid+'" onchange="KitchenUtils.buildTypeProperty( \'parent\', this.value,\''+ nodeUuid +'\');"   ></select> </td>';
	
			inputPar   += '<td id="td_' + nodeUuid + '"></td>';
			inputPar   += '<td><input type="button" name="add" id="addPar_'+nodeUuid+'"  class="btn btn-primary btn-xs"   value="Add" onclick="( new KitchenManagerRender() ).addParentNode(\''+ nodeUuid + '\');">';		
			inputPar   += '<input type="button" name="cancel" value="cancel"  onclick="KitchenUtils.cancelAddPar()"></td></tr>';
				
			tableNodeChildDiv.append(inputPar);		
	
			KitchenUtils.buildTypeList(nodeUuid, 'Parent');
		} else {
...
//	BUILT ADD SIBLING NODE FORM  Based on SELECTED CHILD TYPE
	this.addSibNodeForm = function( nodeUuid ) {
		...
		if($(".addSibNodeRow").length == 0) {
			...
			inputSibl   += '<td>Select Type: <select id="select_sibling_type_'+nodeUuid+'"  onchange="KitchenUtils.buildTypeProperty( \'sibling\', this.value,\''+ nodeUuid +'\');" ></select> </td>';
	
			inputSibl   += '<td id="td_' + nodeUuid + '"></td>';
			inputSibl   += '<td><input type="button" id="addSib_'+nodeUuid+'" name="add"  class="btn btn-primary btn-xs"  value="Add" onclick="( new KitchenManagerRender() ).addSibNode(\''+ nodeUuid + '\');">';		
			inputSibl   += '<input type="button" name="cancel" value="cancel"  onclick="KitchenUtils.cancelAddSib()"></td></tr>';
				
			tableNodeSibDiv.append(inputSibl);		
	
			KitchenUtils.buildTypeList(nodeUuid, 'Sibling');
		} else {
	...
	this.addChildNode = function( nodeuuid){
		...
		var doneFunction = function( dataNode ) {
			...
			var doneFunction = function ( dataEdge ){
				...
				( new KitchenManagerRender() ).addNodeToChildList(nodeuuid, destUuid, dataEdge);
...
//			 prepare the jsonData for creating an edge
			...
			var doneFunction = function ( dataEdge ){
				...
				( new KitchenManagerRender() ).addNodeToParList(nodeuuid, originUuid, dataEdge.type);
			}
    ...
    this.addSibNode   =  function( nodeuuid){
		...
		var doneFunction = function( dataNode ) {    // returned newly created node 
			...
			var doneFunction = function ( dataEdge ){
				...
				( new KitchenManagerRender() ).addNodeToSibList(nodeuuid, destUuid, dataEdge.type);
			}
...
//=================================================================================================================================		
    ...
    this.addNodeToChildList = function ( node, nodetoadd, edgeType){  
		...
		nodeTableChildDiv.append('<tr class="child"><td><a href="#" onclick="">'+  instName + '</a></td><td><a href="#" onclick="KitchenUtils.showEdgeDetails(\''+node+'\',' +edgeType.id + ')">' +edgeType.type + '</td></td></tr>');		     	
    }
    ...
    this.addNodeToParList = function ( node, nodetoadd, edgeType){
		...
		nodeTableParDiv.append('<tr class="parent"><td><a href="#" onclick="">' +  instName + '</a></td><td><a href="#" onclick="KitchenUtils.showConnection(' +rule.id + ')">' + edgeType + '</td></tr>');
    }
 ...
 // display the node Sibling name in the list
    this.addNodeToSibList = function ( node, nodetoadd, edgeType){ 
	    ...
	    var nodeTableSibDiv = $( "#nodeFormSibTable_" + node );
   	    nodeTableSibDiv.append('<tr class="sibling"><td><a href="#" onclick="">' +  instName + '</a></td><td><a href="#" onclick="KitchenUtils.showConnection(' +rule.id + ')">' + edgeType+ '</td></td></tr>');
...
//==========================================================================================================================================	
    this.displayViewEditProperties = function( tempType, key, value  ){
    	...
    	if(!key  ){
			console.log(" no Kitchen uuid  given "+ key );
			return;
		}
   	
    	if(!value){
    		console.log(" no  details given for the Kitchen  "+ key);
		...
		if($.isEmptyObject(tempType.typeProperties)){
			...
			tableEditInstProps = '';
				...
				$.each( tempType.typeProperties, function( pkey, pvalue ) {
					...
					if(pvalue.propertyType == "DATE") { typeInput = "Date"}
					else if(pvalue.propertyType == "INTEGER") {typeInput = "number"}
					else {typeInput = 'text'};
					...
					if( (propValue != null )&& (propValue != 'undefined' ) ){				
						...
						inputpropEditRow +=  '</td><td><input type="'+typeInput+'"   name ="newValue"  value="'+ propValue +'"  />';
			
					} else {
						inputpropViewRow += '<td  width="100px"> NONE </td>';
						
						inputpropEditRow +=  '<input type="hidden" name ="value"     value=""  />'  ;
						inputpropEditRow +=  '</td><td><input type="'+typeInput+'" style="background-color:yellow"    name ="newValue"  value=""  />';	
					}	
				...
				var footerView  = "<tr><td colspan='2'> <input type='button' class='btn btn-primary btn-xs'  value='Go Update ' onclick='( new KitchenManagerRender() ).editKitchen(\"" + key + "\")' /></tr>" ;
				var footerEdit  = "<tr><td colspan='2'> <input type='button' class='btn btn-primary btn-xs'  value='Update' onclick='( new KitchenManagerRender() ).saveUpdateKitchen(\"" + key + "\")' />";
					footerEdit += "<input type='reset' value='Cancel' onclick='KitchenUtils.updateKitchen_cancel(\"" + key + "\")' /></td></tr>";
    ...
    this.buildParentChildrenSiblingDivsForANode = function ( nodeUuid ){
		...
		if(!nodeUuid){
			console.log(" no Kitchen Uuid provided: cannot build the parents/Children/Sibling list  "); 
			return;
		}
		if($.isEmptyObject(nodeMap[nodeUuid])){
			console.log(" Kitchen does not exist in NodeMap -- can not build its Parents/Children/Sibling")
			return;
		}
		var listTypeParents = [];
		...
		if(!$.isEmptyObject(parents) ){
			$.each( parents, function( key, value ) {		
				 if($.inArray(value.TypeName, listTypeParents) == '-1' ){listTypeParents.push(value.TypeName);}
	    		  ...
	    		  nodeTableParDiv.append('<tr class="parent"><td>' +  value.name + '</td><td><a href="#" onclick="KitchenUtils.showEdgeDetails(\''+nodeUuid+'\',' +value.connId + ')">' + value.ruleName + '</a></td></tr>');		
			});
		...
		if(!$.isEmptyObject(children) ){
			$.each( children, function( key, value ) {	
				 if($.inArray(value.TypeName, listTypeChildren) == '-1' ){listTypeChildren.push(value.TypeName);}
					  ...
					  nodeTableChildDiv.append('<tr class="child"><td>'+  value.name + '</td><td><a href="#" onclick="KitchenUtils.showEdgeDetails(\''+nodeUuid+'\',' +value.connId + ')">' + value.ruleName + '</a></td></tr>');  
			});	
		...
		if(!$.isEmptyObject(sibling) ){
			$.each( sibling, function( key, value ) {
				if($.inArray(value.TypeName, listTypeSibling) == '-1' ){listTypeSibling.push(value.TypeName);}
		        	  ...
		        	  nodeTableSibDiv.append('<tr class="sibling"><td>' +  value.name + '</td><td><a href="#" onclick="KitchenUtils.showEdgeDetails(\''+nodeUuid+'\',' +value.connId + ')">' + value.ruleName + '</a></td></tr>');
		    });	
    ...
    this.displayConnectTo = function ( instId ){
		if(!instId){
			console.log(" no Kitchen Uuid provided: cannot display the connect to table  "); 
			return;
		}
		...
		var inputRow = '';
        inputRow   +=  "<tr><td>Connect Kitchen as :<br/><input type='radio' value='asParent' name='typeConnPosition_"+instId+"' onchange=\"KitchenUtils.buildTypeListConnect(\'"+ instId+"\')\"/ >Parent<br/>";					
		inputRow   +=  "<input type='radio' value='asChild' name='typeConnPosition_"+instId+"' onchange=\"KitchenUtils.buildTypeListConnect(\'"+ instId+"\' )\">Child</td></tr>";
    ...
    this.displayLinkTo  = function(instId) {
		...
		var inputRow = '';
        inputRow   +=  "<tr><th><input type='checkbox' value='linkTo'  name='typeLinkPosition_"+instId+"' onchange='KitchenUtils.buildTypeListLink(\""+ instId+"\" )'/ >Link Kitchen To </th></tr>";					
        tableInstlinktoDiv.append(inputRow);
		
	}
    
//========================================================================================================================    
    this.selectedKitchen = function( kitchenId ) {
		
		console.log("Attempting to show :" + kitchenId );					
		if(!kitchenId){
			console.log(" no Kitchen Uuid provided: cannot display related info  "); 
			return;
		}
		var kitchenDetails = nodeMap[kitchenId];
		
		if(!kitchenDetails){
			console.log(" no Kitchen details  were Found " );
			return;
		}	
		
		// hide all other divisions
		$('div.kitchen_detail').hide();
		$('div.form_inst_edgedetails').hide();
		$('div.work_addInstance').hide();	

		$("#kitchen_detail_" +kitchenId).empty();
		
		var instHolderDiv = document.getElementById("kitchen_detail_" +kitchenId);
		KitchenUtils.addDivsForAKitchenDetails (instHolderDiv, kitchenId );
		this.setDefaultsForDivs( kitchenId );		
		...
		var successFunction = function( data ) {
			if( $.isEmptyObject(data) ||   $.isEmptyObject(data.edges)){      // at least the node itself is returned if it has no Parent/Children/Sibling
				...
				NodeUtils.buildNodeDetails ( kitchenId, data);
			}					
		};	
		var failFunction = function( xhr, status, error ) {
			console.log('Not able to load related  Kitchen Details: ' + xhr.status);
			$('#console-log').append("<p style='color:red'>Not able to load related Kitchen Details: "+ xhr.status+"</p>");	
		};	
		var nodeApi = new NodeApis();			
		nodeApi.getRelatedNodesEdges(nodeMap[kitchenId].typeId, kitchenId, successFunction, failFunction);
			
		var typeInfo = typeMapViaId[listTypeIds[0]];
		this.displayViewEditProperties( typeInfo, kitchenId,  kitchenDetails  );
		
		this.buildParentChildrenSiblingDivsForANode( kitchenId );	 
		this.displayConnectTo( kitchenId);
		this.displayLinkTo( kitchenId);
					
		KitchenUtils.showDivision ("form_inst_", kitchenId  );			
		KitchenUtils.showDivision ("form_inst_children_", kitchenId  );		
		KitchenUtils.showDivision ("form_inst_parent_", kitchenId  );	
		KitchenUtils.showDivision ("form_inst_sibling_", kitchenId  );	
		KitchenUtils.showDivision ("form_inst_linkto_", kitchenId  );	
		KitchenUtils.showDivision ("form_inst_connectto_", kitchenId  );
				
		KitchenUtils.showDivision ("kitchen_detail_", kitchenId  );		
		      
	};

//=====================================================================================================      	
	
	this.saveKitchenCreated = function ( kitchenId, properties ){
		
		var data = new NodeJsonObject();
		// set the minimum
		data.init(selectedMetaData.toString(), kitchenId.toString(), properties);
		data.defaultDecorator = "1";
	...
	this.saveUpdateKitchen = function( kitchenId ) {
		if(!kitchenId  ){
			console.log(" no Kitchen uuid  given "+ kitchenId );
			return;
		}
	    	
		// grab the info from the Node property fields 	
		var instEditDiv = $("#form_inst_edit_" + kitchenId );	
					
		var jsonData = {}, nodeProperties = [], newProperties = [];
		var property = {}, newproperty = {}, foundError=false;			

		// grab type Id 
		var typeId         = nodeMap[kitchenId].typeId;	
		if(!typeId){
			console.log(" type is not defined for this node "+ kitchenId);
			return;
		}
		...
		var sysProperties = [];
		property =  NodeUtils.createSysJson(kitchenId );
		...
		$(instEditDiv).find('tr#props').each(function (i, propsTr){	
			$(propsTr).find(':input').each(function(i,field){
			     if( ( field.type  == 'text')||(field.type == 'hidden')||(field.type == 'Date')||(field.type == 'number')){
...
//====================================================================================================	
    ...
    this.editKitchen = function( kitchenId ) {
		if(!kitchenId  ){
			console.log(" no Kitchen uuid  given "+ kitchenId );
			return;
		}
		
		console.log("Attempting to edit :" + kitchenId );			
		KitchenUtils.hideDivision ("form_inst_", kitchenId  );		
		KitchenUtils.showDivision ("form_inst_edit_", kitchenId  );		
		      
	};
			
	this.createKitchen = function(  ) {
		
		console.log("Attempting to create Kitchen :" );	
		...
		this.addFormCreateKitchen( workDiv);					
...
//=====================================================================================================	
	...
	this.addFormCreateKitchen = function (workDiv ){              
		...
		var inputContentList = '';
		 ...
		 var footer  = '<tr align="center"><td colspan="2"><input type="button"  name="submit" class="btn btn-primary btn-xs"   id="addNodeInst_submit_' + listTypeIds[0]+ '" value="Add Instance" onclick="KitchenUtils.saveKitchenInfo('+listTypeIds[0]+');"/>';		
	         footer += '<input type="button"   name="cancel" id="addNodeInst_cancel_' + listTypeIds[0] + '" value="Cancel" onclick="KitchenUtils.createKitchen_cancel(' + listTypeIds[0] + ');"/></td></tr>';   		    
		 $('#addNodeInst_'+listTypeIds[0]+' table ').append(footer);
		 
		 KitchenUtils.showDivision ("work_addInstance", ''  );		 
