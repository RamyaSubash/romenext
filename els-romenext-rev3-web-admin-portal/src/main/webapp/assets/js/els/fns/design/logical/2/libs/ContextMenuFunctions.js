...
ContextMenuFunctions.displayNodeProperties  = function( node, typeId , pos , e){
	...
	node.qtip({
				content : {
					title :  {text: "Type Details",button : true},       
					text : function (){
				...
				events: {
					hide: function(event, api){
						api.destroy();
						if(typeMapViaId[typeId].classification != 'link'  && typeMapViaId[typeId].classification != 'DCT' ) {document.getElementById(typeId).style.border = "";}
...
ContextMenuFunctions.showUpdateType = function() {
...
ContextMenuFunctions.displayLinkProperties  = function( node, ruleId , pos , e){
	...
	node.qtip({
		        id: ruleId,
				content : {	
					title : {text: "Link Details",button : true}, 
					text : function (){
...
ContextMenuFunctions.saveRuleProperties = function(form) {
	...
	var jsonData = {}, ruleProperties = [], property = {}, connname, connId, ruleId;
	// Retrieve type fields from form
	...
	var successFunction = function(data) {
		...
		tdvCy.filter('node[name="' + data.name + '"]').data(data);
		tdvCy.$('#' + data.id + '').data('name', data.name);
		ContextMenuFunctions.CancelAddRuleProperties( data.id )
...
ContextMenuFunctions.showUpdateLink = function(ruleid) {
	...
	var Form, rule, formHeader, formFooter, Props, inputs = '';
	Form = document.createElement('div');
...
ContextMenuFunctions.saveUpdateLinkByGroup = function(form) {
	var jsonData = {},ruleProperties = [], jsonProperty = {}, decos = [], foundError = false,  ruleId;
	var node = {};
