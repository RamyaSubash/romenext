...
function createConnection(connClassification){
	...
	if (connClassification == "parentchild") {
		var img = document.getElementById("img_connect_pc"), list=[];
		if(GlobalUtils.retrieveImgname(img) == "conn_pc.png") {
			img.src = img_path +"design_icons/conn_pc_active.png";
			...
			resetConnVariables();
				...
				var img = document.getElementById("img_connect_link"), list=[];
				if(GlobalUtils.retrieveImgname(img) == "conn_link.png") {
					img.src = img_path +"design_icons/conn_link_active.png";
					console.log("create a link connection");
					ruleSelected = true; //to differentiate between single click and double click
					createRuleClassification = connClassification;
					GlobalUtils.cursor_wait();
				}else{
