						...
						/** *** */
							...
							([
									...
									(function(module, exports,
										...
										// polyfill for options objects etc.

										module.exports = Object.assign != null ? Object.assign
												.bind(Object)
												: function(tgt) {
													for (var _len = arguments.length, srcs = Array(_len > 1 ? _len - 1
									...
									(function(module, exports,
										...
										function resetGestures() {
											this.cy
													.zoomingEnabled(
															this.lastZoomingEnabled)
													.panningEnabled(
															this.lastPanningEnabled)
													.boxSelectionEnabled(
															this.lastBoxSelectionEnabled);
										...
										function saveGestureState() {
											...
											this.lastPanningEnabled = cy
													.panningEnabled();
											this.lastZoomingEnabled = cy
													.zoomingEnabled();
											this.lastBoxSelectionEnabled = cy
													.boxSelectionEnabled();
									...
									(function(module, exports,
										...
										var defaults = {
											preview : true, // whether to show
															// added edges
															// preview before
															// releasing
															// selection
											hoverDelay : 150, // time spent
																// hovering over
																// a target node
																// before it is
																// considered
																// selected
											handleNodes : 'node', // selector/filter
																	// function
																	// for
																	// whether
																	// edges can
																	...
																	// given
																	// node
											handlePosition : 'middle top', // sets
																			// the
																			// position
																			// of
																			// the
																			...
																			// "middle
																			// top"
											handleInDrawMode : false, // whether
																		// to
																		// show
																		// the
																		// handle
																		...
																		// draw
																		// mode
											edgeType : function edgeType(
													sourceNode, targetNode) {
												// can return 'flat' for flat
												// edges between nodes or 'node'
												// for intermediate node between
												// them
												// returning null/undefined
												// means an edge can't be added
											...
											loopAllowed : function loopAllowed(
												...
												// return whether edges from
												// itself to itself are allowed
												return false;
											},
											nodeLoopOffset : -50, // offset
																	// for
																	// edgeType:
																	// 'node'
																	// loops
											nodeParams : function nodeParams(
													sourceNode, targetNode) {
												// for edges between the
												// specified source and target
												// return element object to be
												// passed to cy.add() for
												// intermediary node
												return {};
											},
											edgeParams : function edgeParams(
													sourceNode, targetNode, i) {
												// for edges between the
												// specified source and target
												// return element object to be
												// passed to cy.add() for edge
												// NB: i indicates edge index in
												// case of edgeType: 'node'
												return {};
											},
											...
											hide : function hide(sourceNode) {
												// fired when the handle is
												// hidden
											},
											start : function start(sourceNode) {
												// fired when edgehandles
												// interaction starts (drag on
												// handle)
											},
											complete : function complete(
													sourceNode, targetNode,
													addedEles) {
												// fired when edgehandles is
												// done and elements are added
											},
											stop : function stop(sourceNode) {
												// fired when edgehandles
												// interaction is stopped
												// (either complete with added
												// edges or incomplete)
											},
											cancel : function cancel(
													sourceNode,
													cancelledTargets) {
												// fired when edgehandles are
												// cancelled (incomplete
												// gesture)
											},
											hoverover : function hoverover(
													sourceNode, targetNode) {
												// fired when a target is
												// hovered
											},
											hoverout : function hoverout(
													sourceNode, targetNode) {
												// fired when a target isn't
												// hovered anymore
											},
											previewon : function previewon(
													sourceNode, targetNode,
													previewEles) {
												// fired when preview is shown
											},
											previewoff : function previewoff(
													sourceNode, targetNode,
													previewEles) {
									...
									(function(module, exports,
										...
										function toggleDrawMode(bool) {
											...
											this.drawMode = bool != null ? bool
													: !this.drawMode;

											if (this.drawMode) {
												this.prevUngrabifyState = cy
														.autoungrabify();

												cy.autoungrabify(true);

												if (!options.handleInDrawMode
														&& this.handleShown()) {
													this.hide();
												}
												...
												cy
														.autoungrabify(this.prevUngrabifyState);
									...
									(function(module, exports,
										...
										function addClassesToEleJson(json,
												classes) {
											if (json.classes) {
										...
										function makeEdges() {
											var preview = arguments.length > 0
													&& arguments[0] !== undefined ? arguments[0]
													: false;
											var cy = this.cy, options = this.options, presumptiveTargets = this.presumptiveTargets, previewEles = this.previewEles;
											...
											// just remove preview class if we
											// already have the edges
											if (!preview && options.preview) {
												previewEles
														.removeClass('eh-preview');
											...
											if (edgeType === 'node') {
												...
												added = added.merge(interNode)
														.merge(source2inter)
														.merge(inter2target);
											} else {
												// flat
												var source2target = cy
														.add(addClassesToEleJson(
																assign(
																		{
																			group : 'edges',
										...
										function previewShown() {
											return this.previewEles.nonempty()
													&& this.previewEles
															.inside();
										}
										...
										function handleShown() {
											return this.handleNode.nonempty()
													&& this.handleNode.inside();
										}
										...
										function setHandleFor(node) {
											...
											var options = this.options, cy = this.cy;

											var p = node.position();
											var h = node.outerHeight();
											var w = node.outerWidth();

											// store how much we should move the
											// handle from origin(p.x, p.y)
											var moveX = 0;
											var moveY = 0;

											// grab axis's
											var axes = options.handlePosition
													.toLowerCase().split(' ');
											var axisX = axes[0];
											var axisY = axes[1];

											// based on handlePosition move
											// left/right/top/bottom.
											// Middle/middle will just be normal
											if (axisX === 'left') {
												moveX = -(w / 2);
											} else if (axisX === 'right') {
												moveX = w / 2;
										...
										function updateEdge() {
											...
											// can't draw a line without having
											// the starting node
											if (!sourceNode) {
												return;
											}
											...
											if (!ghostNode
													|| ghostNode.length === 0
													|| ghostNode.removed()) {
												ghostEles = this.ghostEles = cy
														.collection();

												cy
														.batch(function() {
															ghostNode = _this2.ghostNode = cy
																	.add({
																		group : 'nodes',
									...
									(function(module, exports,
										...
										function canStartOn(node) {
											var options = this.options, previewEles = this.previewEles, ghostEles = this.ghostEles, handleNode = this.handleNode;
											...
											var userFilter = function userFilter(
													el) {
												return el
														.filter(options.handleNodes).length > 0;
											};
											...
											return enabled
													&& !active
													&& !grabbingNode
													&& (node == null || !isTemp(node)
															&& userFilter(node));
										}

										function canStartDrawModeOn(node) {
											return this.canStartOn(node)
													&& this.drawMode;
										}
										...
										function show(node) {
											var options = this.options, drawMode = this.drawMode;

											if (!this.canStartOn(node)
													|| drawMode
													&& !options.handleInDrawMode) {
												return;
											}
										...
										function preview(target) {
											...
											if (!this.active
													|| target
															.same(this.handleNode)) {
												return;
											}

											var options = this.options, sourceNode = this.sourceNode, ghostNode = this.ghostNode, presumptiveTargets = this.presumptiveTargets, previewEles = this.previewEles;
											...
											this.previewTimeout = setTimeout(
													function() {
														...
														target
																.addClass('eh-presumptive-target');
										...
										function unpreview(target) {
											...
											var previewTimeout = this.previewTimeout, sourceNode = this.sourceNode, previewEles = this.previewEles;
											...
											target
													.removeClass('eh-preview eh-target eh-presumptive-target');

											this.removePreview(source, target);

											this.emit('hoverout', this.mp(),
													source, target);
											this
													.emit('previewoff', this
															.mp(), source,
															target, previewEles);
										...
										function stop() {
											...
											var sourceNode = this.sourceNode, targetNode = this.targetNode, handleNode = this.handleNode, ghostEles = this.ghostEles, presumptiveTargets = this.presumptiveTargets;
											...
											targetNode
													.removeClass('eh-target eh-preview eh-hover');
											presumptiveTargets
													.removeClass('eh-presumptive-target');
									...
									(function(module, exports,
										...
										proto.clearCollections = function() {
											...
											this.presumptiveTargets = cy
													.collection();
										};

										[ cyGesturesToggle, cyListeners,
												drawMode, drawing, enabling,
												gestureLifecycle, listeners ]
												.forEach(extend);
									...
									(function(module, exports,
										...
										function removeListeners() {
											for (var i = this.listeners.length - 1; i >= 0; i--) {
												...
												this.removeListener(l.target,
														l.event, l.selector,
														l.callback, l.options);
											}
										...
										function getListener(target, event,
											...
											if ((typeof selector === 'undefined' ? 'undefined'
													: _typeof(selector)) !== _typeof('')) {
												callback = selector;
												options = callback;
												selector = null;
											}
										...
										function addListener(target, event,
												selector, callback, options) {
											var l = getListener(target, event,
													selector, callback, options);
									...
									(function(module, exports,
										...
										// registers the extension on a
										// cytoscape lib ref
										var register = function register(
												cytoscape) {
											if (!cytoscape) {
												return;
											} // can't register if cytoscape
												// unspecified

											cytoscape('core', 'edgehandles',
													impl); // register with
															// cytoscape.js
										};

										if (typeof cytoscape !== 'undefined') {
											// expose to global cytoscape (i.e.
											// window.cytoscape)
